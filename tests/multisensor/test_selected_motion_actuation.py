...
from webots_adapter import VirtualMQTTController
...
robot = Driver()
motion = VirtualMQTTController(robot, "test")
...
try:
    ...
    while True:
        ...
        if maneuver == "STOP" or selected_speed <= 0.0:
            motion.stop()
        else:
            test_angle = angles.get(maneuver)

            if test_angle is None:
                motion.stop()
            else:
                test_speed = min(selected_speed, 0.5)
                motion.send_motion_command(test_angle, test_speed)

        if cycle >= 30:
            motion.stop()
            print("[PASS] Selected maneuver actuation test completed")
            break
